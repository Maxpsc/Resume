		...
		"projects":[
			{
				...
				"duration": ["2020.1", "至今"],
				"description": [
					"主导通用工具函数、hooks和UI组件的整合，并负责核心逻辑开发；",
					"工具函数包括各类格式化函数、表单校验和针对业务、容器的方法封装；",
					"结合银行SPA应用特点和统一前端技术栈（react+antdm+ts+hooks）的前提，实现基于hooks的状态管理框架；",
					"通用request，中间件机制参考koa洋葱模型，并实现监控、轮询、错误处理、免登等中间件；",
					"输出mybank-hooks, mybank-utils, mybank-components作为资产沉淀；",
					"解决H5逻辑分散、管理混乱、外包质量不可控、产品对前端现有能力缺乏认知等问题，一定程度上做到质量可控，开发提效（快速创建、代码复用），视觉、研发流程的统一规范，且落地的前端资产可以赋能业务，提高产品视觉对前端能力的认知；",
					"计划作为基础物料，配合逻辑、UI编排方案，组成综合金融前端解决方案；",
				]
			},
			{
				"name": "移动端收银台",
				"duration": ["2021.2", "至今"],
				"description": [
					"整体设计和唯一开发者，实现网商银行个人转账收银台",
					"收付方信息、可用资产列表和交易内容展示，提供完整的前端收银交互能力",
					"分层抽象，对外输出sdk、组件、页面三种接入方式，并封装监控",
					"基于区块-h5data实现的支持根据渠道定制可用资产和一定程度的样式、文案配置",
				]
			},
			{
				"name": "个人会员开户sdk",
				...
				"description": [
					"网商全功能二类户在多产品覆盖，弱感知开户；",
					"作为核心开发者，实现开户逻辑内聚，比照服务端能力，原子化、产品化输出；",
					"统筹管理开户能力，基于埋点和监控实现分渠道的数据分析和线上监控；",
					"场景接入，简化调用api，多端容器兼容；"
				]
			},
			{
				...
				"description": [
					"受监管压力和政策变化频繁的影响，网商存款产品对客展示形态多变，需要快速搭建相似产品的能力",
					"网商银行定活宝、随意存等存款产品的核心逻辑抽象；",
					"主要为UI渲染配置，诸如首页、存单列表的区块约定和配置，逻辑层差异较小，主要由接口控制链路；",
